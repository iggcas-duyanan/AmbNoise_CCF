...
%                 disp(['Using 's,pzfile.name])
        ...
        Z2raw = rm_SACPZ(Z2raw,z,p,c,dt2);
          ...
          %  end
            ...
            nwin = floor((24/hour_length)*1.5)+2; % 8 3 hour segments           
            % nwin = 46; % 46 30 minute long overlapping time segments          
            win_length = hour_length*60*60; % length of individual windows.     
            win_start = 1;
			for iwin = 1:nwin
				clear tcut Z1 Z2 fftZ1 fftZ2

				% cut in time
				pts_begin = win_length*.5*(iwin-1)+1+Nstart;
				pts_end = pts_begin+win_length;
				
				if pts_begin > length(Z1raw) || pts_begin > length(Z2raw) || pts_end > length(Z1raw) || pts_end > length(Z2raw)
					disp('Points greater than the data');
					continue
				end
            ...
            checknan1 = find(isnan(fftZ1) == 1);                                
            checknan2 = find(isnan(fftZ2) == 1);                                
            if length(checknan1) > 1 || length(checknan2) > 1                   
                                                                                
            continue                                                            
            end                                                                 
                                                                                
            % calcaulate daily CCF and stack                                    
